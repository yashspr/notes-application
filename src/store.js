...
export default new Vuex.Store({
		...
		ADD_DESCRIPTION(state, { fileID, description }) {
			state.notes.forEach((note) => {
				if (note.fileID === fileID)
					// state.notes.fileID.description = description;
	...
	actions: {
		fetchNotes({ commit }) {
			EventService.fetchNotes().then((response) => {
		...
		fetchDescription({ commit }, fileID) {
			EventService.fetchDescription(fileID).then((response) => {
				commit('ADD_DESCRIPTION', { fileID, description: response.data });
			});
		},
		syncNotes({ commit, state }) {
			if (
				state.updateNotes.length === 0 &&
				state.deleteNotes.length === 0 &&
				state.newNotes.length === 0
			) {
				alert('nothing to update/sync');
			} else if (state.deleteNotes.length > 0) {
				EventService.deleteNotes(state.deleteNotes).then((response) => {
				...
				if (state.newNotes.length > 0) {
					EventService.sendNewNotes(state.newNotes).then((response) => {
				...
				} else if (state.updateNotes.length > 0) {
					EventService.sendUpdateNotes(state.updateNotes).then((response) => {
						console.log(response);
